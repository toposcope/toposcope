...
Current release: `0.4.7` · [MIT Expat license](LICENSE)

## Quick start

The packaged install is the recommended path for operators. Download these four files from the [v0.4.7 release](https://github.com/toposcope/toposcope/releases/tag/v0.4.7) into one directory:
...
The deployment pins `ghcr.io/toposcope/toposcope:0.4.7`; it never uses `:latest`. Anonymous image pulls need no registry login.
...
| Probes | `POST /v1/probes` | Attach `up=0`/`up=1` or pull one status URL; overlay `metric=up` |

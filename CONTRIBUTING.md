...
`package.json` is the product version. **0.4** is shipped (`0.4.4`). Series cuts are GitHub milestones. A patch is one release. Changelog entries are the history; they are not a second spec. A `v*` tag publishes the image and zip; GitHub release notes are that version’s changelog section.

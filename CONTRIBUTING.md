...
`bun run load:hunt` plants a named billing v0.9 hunt for marketing capture: mark in the middle of a 1h window, `level:error service:billing` rises after, three first-seen framed bugs on `billing-1` / `billing-2` / `billing-3`, with `version` / `customer` / `flag` on the row (`cols=`). Truncate first. Writes `/tmp/toposcope-hunt.json` (`from` / `to` / `q`) so a later `range=1h` does not slide off the data.
